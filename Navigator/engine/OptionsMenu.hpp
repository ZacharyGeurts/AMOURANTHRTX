...
namespace Rendering {
    ...
    inline int     MAX_RAY_RECURSION         = 8;         // Bounce limit — higher = more noise but realism
}

// ── WINDOW & DISPLAY (SDL3 handles DPI/HDR — we just set sane defaults)
namespace Window {
    inline int      DEFAULT_WIDTH            = 3840;
    inline int      DEFAULT_HEIGHT           = 2160;
...
namespace Audio {
    ...
    inline int     AUDIO_CHANNELS           = 2;          // 1=mono, 2=stereo, 6=5.1, 8=7.1
    inline int     AUDIO_SAMPLE_RATE        = 48000;      // Common rates: 44100, 48000, 96000
    inline int     AUDIO_BUFFER_SIZE        = 2048;       // Samples per buffer — smaller = lower latency, larger = more stable
