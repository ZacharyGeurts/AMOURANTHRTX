...
RenderMode4::RenderMode4(VulkanRTX& rtx, uint32_t width, uint32_t height)
    ...
    LOG_INFO_CAT("RenderMode4", "{}VALHALLA MODE 4 INIT — {}×{} — OVERCLOCK ENGAGED{}", VALHALLA_GOLD, width, height, RESET);
    initResources();
    LOG_SUCCESS_CAT("RenderMode4", "{}Mode 4 Initialized — {}×{} — Max Performance{}", CRIMSON_MAGENTA, width, height, RESET);
}

RenderMode4::~RenderMode4() {
    LOG_INFO_CAT("RenderMode4", "Destructor invoked — Releasing resources");
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    LOG_DEBUG_CAT("RenderMode4", "Mode 4 Resources Released — OVERCLOCK SECURED");
}

void RenderMode4::initResources() {
    LOG_INFO_CAT("RenderMode4", "initResources() — Creating buffers and images");
    auto& ctx = RTX::g_ctx();
    ...
    LOG_SUCCESS_CAT("RenderMode4", "initResources complete — OVERCLOCK ready");
}
...
void RenderMode4::accumulateAndToneMap(VkCommandBuffer cmd) {
    ...
    VkClearColorValue gold = {};
    gold.float32[0] = 1.0f;
    gold.float32[1] = 0.84f;
    gold.float32[2] = 0.0f;
    gold.float32[3] = 1.0f;
    vkCmdClearColorImage(cmd, *outputImage_, VK_IMAGE_LAYOUT_GENERAL, &gold, 1, &barrier.subresourceRange);
}

void RenderMode4::onResize(uint32_t width, uint32_t height) {
    LOG_INFO_CAT("RenderMode4", "onResize() — New: {}×{}", width, height);
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    accumImage_.reset();
    outputImage_.reset();
