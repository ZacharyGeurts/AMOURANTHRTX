...
RenderMode6::RenderMode6(VulkanRTX& rtx, uint32_t width, uint32_t height)
    ...
    LOG_INFO_CAT("RenderMode6", "{}VALHALLA MODE 6 INIT — {}×{} — ULTRA TRACE ENGAGED{}", LILAC_LAVENDER, width, height, RESET);
    initResources();
    LOG_SUCCESS_CAT("RenderMode6", "{}Mode 6 Initialized — {}×{} — Full Spectral Path Tracing{}", LILAC_LAVENDER, width, height, RESET);
}

RenderMode6::~RenderMode6() {
    LOG_INFO_CAT("RenderMode6", "Destructor invoked — Releasing resources");
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    LOG_DEBUG_CAT("RenderMode6", "Mode 6 Resources Released — ULTRA TRACE SECURED");
}

void RenderMode6::initResources() {
    LOG_INFO_CAT("RenderMode6", "initResources() — Creating buffers and images");
    auto& ctx = RTX::g_ctx();
    ...
    LOG_SUCCESS_CAT("RenderMode6", "initResources complete — ULTRA TRACE ready");
}
...
void RenderMode6::accumulateAndToneMap(VkCommandBuffer cmd) {
    ...
    VkClearColorValue cosmic = {};
    cosmic.float32[0] = 0.1f;
    cosmic.float32[1] = 0.2f;
    cosmic.float32[2] = 0.8f;
    cosmic.float32[3] = 1.0f;
    vkCmdClearColorImage(cmd, *outputImage_, VK_IMAGE_LAYOUT_GENERAL, &cosmic, 1, &barrier.subresourceRange);
}

void RenderMode6::onResize(uint32_t width, uint32_t height) {
    LOG_INFO_CAT("RenderMode6", "onResize() — New: {}×{}", width, height);
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    accumImage_.reset();
    outputImage_.reset();
