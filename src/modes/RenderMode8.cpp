...
RenderMode8::RenderMode8(VulkanRTX& rtx, uint32_t width, uint32_t height)
    ...
    LOG_INFO_CAT("RenderMode8", "{}VALHALLA MODE 8 INIT — {}×{} — HYPERREAL ENGAGED{}", LILAC_LAVENDER, width, height, RESET);
    initResources();
    LOG_SUCCESS_CAT("RenderMode8", "{}Mode 8 Initialized — {}×{} — Photorealistic RT{}", LILAC_LAVENDER, width, height, RESET);
}

RenderMode8::~RenderMode8() {
    LOG_INFO_CAT("RenderMode8", "Destructor invoked — Releasing resources");
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    LOG_DEBUG_CAT("RenderMode8", "Mode 8 Resources Released — HYPERREAL SECURED");
}

void RenderMode8::initResources() {
    LOG_INFO_CAT("RenderMode8", "initResources() — Creating buffers and images");
    auto& ctx = RTX::g_ctx();
    ...
    BUFFER_CREATE(uniformBuf_, uniformSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, "RenderMode8 Uniform");
    ...
    BUFFER_CREATE(accumulationBuf_, accumSize_, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, "RenderMode8 Accum");
    ...
    LOG_SUCCESS_CAT("RenderMode8", "initResources complete — HYPERREAL ready");
}
...
void RenderMode8::accumulateAndToneMap(VkCommandBuffer cmd) {
    ...
    VkClearColorValue crimson = {};
    crimson.float32[0] = 0.86f;
    crimson.float32[1] = 0.08f;
    crimson.float32[2] = 0.24f;
    crimson.float32[3] = 1.0f;
    vkCmdClearColorImage(cmd, *outputImage_, VK_IMAGE_LAYOUT_GENERAL, &crimson, 1, &barrier.subresourceRange);
}

void RenderMode8::onResize(uint32_t width, uint32_t height) {
    LOG_INFO_CAT("RenderMode8", "onResize() — New: {}×{}", width, height);
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    accumImage_.reset();
    outputImage_.reset();
