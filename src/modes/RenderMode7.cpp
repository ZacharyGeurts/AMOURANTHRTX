...
RenderMode7::RenderMode7(VulkanRTX& rtx, uint32_t width, uint32_t height)
    ...
    LOG_INFO_CAT("RenderMode7", "{}VALHALLA MODE 7 INIT — {}×{} — QUANTUM DENOISE ENGAGED{}", LILAC_LAVENDER, width, height, RESET);
    initResources();
    LOG_SUCCESS_CAT("RenderMode7", "{}Mode 7 Initialized — {}×{} — AI-Powered Denoiser{}", LILAC_LAVENDER, width, height, RESET);
}

RenderMode7::~RenderMode7() {
    LOG_INFO_CAT("RenderMode7", "Destructor invoked — Releasing resources");
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    LOG_DEBUG_CAT("RenderMode7", "Mode 7 Resources Released — QUANTUM DENOISE SECURED");
}

void RenderMode7::initResources() {
    LOG_INFO_CAT("RenderMode7", "initResources() — Creating buffers and images");
    auto& ctx = RTX::g_ctx();
    ...
    BUFFER_CREATE(uniformBuf_, uniformSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, "RenderMode7 Uniform");
    ...
    BUFFER_CREATE(accumulationBuf_, accumSize_, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, "RenderMode7 Accum");
    ...
    LOG_SUCCESS_CAT("RenderMode7", "initResources complete — QUANTUM DENOISE ready");
}
...
void RenderMode7::updateUniforms(float deltaTime) {
    void* data = nullptr;
...
void RenderMode7::traceRays(VkCommandBuffer cmd) {
    rtx_.recordRayTrace(cmd, {width_, height_}, *outputImage_, *outputView_);
}

void RenderMode7::accumulateAndToneMap(VkCommandBuffer cmd) {
    ...
    VkClearColorValue violet = {};
    violet.float32[0] = 0.93f;
    violet.float32[1] = 0.51f;
    violet.float32[2] = 0.93f;
    violet.float32[3] = 1.0f;
    vkCmdClearColorImage(cmd, *outputImage_, VK_IMAGE_LAYOUT_GENERAL, &violet, 1, &barrier.subresourceRange);
}

void RenderMode7::onResize(uint32_t width, uint32_t height) {
    LOG_INFO_CAT("RenderMode7", "onResize() — New: {}×{}", width, height);
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    accumImage_.reset();
    outputImage_.reset();
