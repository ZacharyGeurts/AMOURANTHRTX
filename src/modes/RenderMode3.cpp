...
RenderMode3::RenderMode3(VulkanRTX& rtx, uint32_t width, uint32_t height)
    ...
    LOG_INFO_CAT("RenderMode3", "{}VALHALLA MODE 3 INIT — {}×{} — DENOISING ENGAGED{}", EMERALD_GREEN, width, height, RESET);
    initResources();
    LOG_SUCCESS_CAT("RenderMode3", "{}Mode 3 Initialized — {}×{} — SVGF Active{}", THERMO_PINK, width, height, RESET);
}

RenderMode3::~RenderMode3() {
    LOG_INFO_CAT("RenderMode3", "Destructor invoked — Releasing resources");
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    LOG_DEBUG_CAT("RenderMode3", "Mode 3 Resources Released — DENOISE SECURED");
}

void RenderMode3::initResources() {
    LOG_INFO_CAT("RenderMode3", "initResources() — Creating buffers and images");
    auto& ctx = RTX::g_ctx();
    ...
    LOG_SUCCESS_CAT("RenderMode3", "initResources complete — SVGF ready");
}
...
void RenderMode3::accumulateAndToneMap(VkCommandBuffer cmd) {
    ...
    VkClearColorValue lime = {};
    lime.float32[0] = 0.5f;
    lime.float32[1] = 1.0f;
    lime.float32[2] = 0.0f;
    lime.float32[3] = 1.0f;
    vkCmdClearColorImage(cmd, *outputImage_, VK_IMAGE_LAYOUT_GENERAL, &lime, 1, &barrier.subresourceRange);
}

void RenderMode3::onResize(uint32_t width, uint32_t height) {
    LOG_INFO_CAT("RenderMode3", "onResize() — New: {}×{}", width, height);
    if (uniformBuf_) BUFFER_DESTROY(uniformBuf_);
    if (accumulationBuf_) BUFFER_DESTROY(accumulationBuf_);
    accumImage_.reset();
    outputImage_.reset();
