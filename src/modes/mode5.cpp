// mode5.cpp
// Implementation of renderMode5 for AMOURANTH RTX Engine to draw five moving mirror balls.
...
void renderMode5(const UE::AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        pushConstants.color = (i == 0) ? glm::vec4(0.8f, 0.8f, 0.8f, 1.0f) :
                             ...
                             (i == 3) ? glm::vec4(0.9f, 0.5f, 0.3f, 1.0f) :
                                        glm::vec4(0.5f, 0.5f, 0.9f, 1.0f); // Blue
