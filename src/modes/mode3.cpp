// mode3.cpp
// Implementation of renderMode3 for AMOURANTH RTX Engine to draw three moving mirror balls.
...
void renderMode3(const UE::AMOURANTH* amouranth, [[maybe_unused]] uint32_t imageIndex, VkBuffer vertexBuffer, VkCommandBuffer commandBuffer,
    ...
    // Bind vertex buffer (assumes three vertices)
    ...
    // Draw three mirror balls with different orbits
    for (int i = 0; i < 3; ++i) {
        // Vary angle and radius for each ball
        float angle = time * (1.0f + i * 0.5f); // Different speeds: 1, 1.5, 2 rad/s
        float radius = 1.0f + i * 0.5f; // Different radii: 1, 1.5, 2
        ...
        pushConstants.color = (i == 0) ? glm::vec4(0.8f, 0.8f, 0.8f, 1.0f) : // Silver
                              (i == 1) ? glm::vec4(1.0f, 0.84f, 0.0f, 1.0f) : // Gold
                                         glm::vec4(0.8f, 0.5f, 0.2f, 1.0f); // Bronze
