    ...
    [[nodiscard]] SDLWindowPtr create(const char* title, int width, int height, Uint32 flags)
    {
        ...
        // FIXED: Conditionally enable Vulkan — check system support first to avoid "VK_KHR_surface missing" crash
        bool vulkanSupported = false;
        bool libLoaded = SDL_Vulkan_LoadLibrary(nullptr);  // SDL3: Returns bool (success), not char*
        if (libLoaded) {
            // Quick check for VK_KHR_surface (core for SDL_WINDOW_VULKAN)
            uint32_t extCount = 0;
            ...
            SDL_Vulkan_UnloadLibrary();  // Unload after check
            LOG_INFO_CAT("Window", "Vulkan supported (VK_KHR_surface present) — enabling SDL_WINDOW_VULKAN");
        } else {
            LOG_WARN_CAT("Window", "Vulkan library load failed: {} — falling back to non-Vulkan window", SDL_GetError());
        }

        flags |= SDL_WINDOW_HIGH_PIXEL_DENSITY;
        if (vulkanSupported) {
            flags |= SDL_WINDOW_VULKAN;
            LOG_INFO_CAT("Window", "SDL_WINDOW_VULKAN enabled — ready for RTX forge");
        }
        ...
        if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS) == 0) {  // FIXED: != 0 → == 0 for success check
        ...
        LOG_SUCCESS_CAT("Window", "Window created @ {:p} — {}x{} (Vulkan: {})", static_cast<void*>(raw), width, height, vulkanSupported ? "YES" : "NO");
