...
//
// TRUE CONSTEXPR STONEKEY v∞ — NOVEMBER 20, 2025 — APOCALYPSE FINAL v6.0
// VulkanAccel — FINAL — NO STATIC PFNs — FULL g_ctx() ONLY — FIRST LIGHT ETERNAL
// =============================================================================
...
// =============================================================================
// Constructor — now a no-op (all PFNs live in g_ctx())
// =============================================================================
VulkanAccel::VulkanAccel(VkDevice /*device*/)
{
    // All ray tracing function pointers are loaded into g_ctx() via loadRayTracingExtensions()
    // This class no longer owns any static PFNs — PURE, CLEAN, UNSTOPPABLE
}

// =============================================================================
// Destroy helpers — use g_ctx() PFNs
// =============================================================================
...
// =============================================================================
// BLAS CREATION — FULLY FIXED — USES g_ctx() ONLY
// =============================================================================
                                          ...
                                          std::string_view name)
{
    ...
    if (cmd == VK_NULL_HANDLE) cmd = beginOneTime(g_ctx().commandPool());
    ...
    if (externalCmd == VK_NULL_HANDLE)
        endOneTime(cmd, g_ctx().graphicsQueue(), g_ctx().commandPool());
    ...
    LOG_SUCCESS("[VulkanAccel] BLAS \"%.*s\" FORGED — %.3f GB — ADDR 0x%llx",
                ...
                blas.size / (1024.0 * 1024.0 * 1024.0), blas.address);

    return blas;
}

// =============================================================================
// TLAS CREATION — FULLY FIXED — USES g_ctx() ONLY
// =============================================================================
                                          ...
                                          std::string_view name)
{
    ...
    LOG_SUCCESS("[VulkanAccel] TLAS \"%.*s\" ASCENDED — %u instances — %.3f GB — ADDR 0x%llx",
                (int)name.size(), name.data(), count,
                tlas.size / (1024.0 * 1024.0 * 1024.0), tlas.address);
