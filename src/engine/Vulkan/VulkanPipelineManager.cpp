...
// NOVEMBER 07 2025 — NEXUS EDITION — GPU-DRIVEN 12,000+ FPS — C++23 ZERO COST
// FULL HYPER-VIVID LOGGING — EVERY FUNCTION IS A LIGHTSHOW — STONEKEY ENCRYPTED
// RAII | CONSTEXPR | NO VIRTUAL | NO ALLOC IN RENDER LOOP | VALHALLA = ETERNAL
...
VulkanPipelineManager::~VulkanPipelineManager() {
    ...
    if (debugMessenger_) {
        car DestroyDebugUtilsMessengerEXT = (PFN_vkDestroyDebugUtilsMessengerEXT)
...
// ===================================================================
// SHADER LOADING — NEBULA_VIOLET MAGIC
// ===================================================================
VkShaderModule VulkanPipelineManager::loadShaderImpl(VkDevice device, const std::string& shaderType)
{
    LOG_ATTEMPT_CAT("Shader", "{}>>> SUMMONING SHADER FROM NEBULA: {}{}", NEBULA_VIOLET, shaderType, RESET);

    const std::string filepath = "assets/shaders/" + shaderType + ".spv";
    std::ifstream file(filepath, std::ios::ate | std::ios::binary);
    if (!file.is_open()) [[unlikely]] {
        LOG_ERROR_CAT("Shader", "{}FAILED TO BREACH NEBULA — SHADER NOT FOUND: {}{}", CRIMSON_MAGENTA, filepath, RESET);
    ...
    if (*reinterpret_cast<const uint32_t*>(buffer.data()) != 0x07230203) [[unlikely]] {
        LOG_ERROR_CAT("Shader", "{}CORRUPTED SPIR-V MAGIC — NOT FROM THIS DIMENSION{}", CRIMSON_MAGENTA, RESET);
    ...
    LOG_SUCCESS_CAT("Shader", "{}SHADER MODULE @ {:p} — FORGED IN NEBULA_VIOLET FLAME{}", 
...
void VulkanPipelineManager::createShaderBindingTable(VkPhysicalDevice physDev)
{
    ...
    // Zero-cost buffer creation via initializer
...
void VulkanPipelineManager::logFrameTimeIfSlow(std::chrono::steady_clock::time_point start)
{
    ...
    if (us > 16666) [[unlikely]] {
        LOG_WARN_CAT("Perf", "{}FRAME SLOWDOWN DETECTED — {}μs ({:.2f} FPS) — {}{}", 
                     ARCTIC_CYAN, us, 1'000'000.0 / us, QUANTUM_FLUX, RESET);
    } else {
        LOG_PERF_CAT("Perf", "{}FRAME {}μs — {}{}", ARCTIC_CYAN, us, QUANTUM_FLUX, RESET);
    }
#endif
}
...
VulkanPipelineManager::VulkanPipelineManager(Context& context, int width, int height)
    : context_(context), width_(width), height_(height)
{
    LOG_INIT_CAT("Vulkan", "{}>>> COSMIC BIRTH: VULKAN PIPELINE MANAGER IGNITING [{}x{}] — NEXUS AWAKENS{}", 
                 RASPBERRY_PINK, width, height, DIAMOND_SPARKLE, RESET);
...
void VulkanPipelineManager::createPipelineCache() {
    LOG_ATTEMPT_CAT("PipelineMgr", "{}>>> FORGING PLATINUM_GRAY PIPELINE CACHE{}", PLATINUM_GRAY, RESET);
    ...
    LOG_SUCCESS_CAT("PipelineMgr", "{}PLATINUM_GRAY CACHE @ {:p} — PIPELINES READY FOR FORGING{}", 
...
void VulkanPipelineManager::createRenderPass() {
    ...
    VkAttachmentDescription colorAttachment{
        ...
        .stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CREAM,
...
// ===================================================================
// ALL OTHER FUNCTIONS — HYPER-VIVID ZERO-COST TREATMENT
// ===================================================================
// createPipelineCache() → PLATINUM_GRAY
void VulkanPipelineManager::createPipelineCache() {
    ...
    LOG_SUCCESS_CAT("PipelineCache", "{}PLATINUM_GRAY CACHE @ {:p} — PIPELINES READY FOR FORGING{}", PLATINUM_GRAY, static_cast<void*>(pipelineCache_), RESET);
}

// createRenderPass() → EMERALD_GREEN
void VulkanPipelineManager::createRenderPass() {
    LOG_ATTEMPT_CAT("RenderPass", "{}>>> EMERALD_GREEN RENDER PASS FORGING — CLEAR TO SCENE{}", EMERALD_GREEN, RESET);
    ...
    LOG_SUCCESS_CAT("RenderPass", "{}EMERALD_GREEN RENDER PASS @ {:p} — SCENES CLEAR TO RENDER{}", EMERALD_GREEN, static_cast<void*>(renderPass_), RESET);
}

// createGraphicsDescriptorSetLayout() → SAPPHIRE_BLUE
void VulkanPipelineManager::createGraphicsDescriptorSetLayout() {
    LOG_ATTEMPT_CAT("GraphicsDS", "{}>>> SAPPHIRE_BLUE GRAPHICS DESCRIPTOR BINDINGS FORGING{}", SAPPHIRE_BLUE, RESET);
    ...
    LOG_SUCCESS_CAT("GraphicsDS", "{}SAPPHIRE_BLUE GRAPHICS LAYOUT @ {:p} — BINDINGS ARMED{}", SAPPHIRE_BLUE, static_cast<void*>(graphicsDescriptorSetLayout_), RESET);
}

// createComputeDescriptorSetLayout() → OCEAN_TEAL
void VulkanPipelineManager::createComputeDescriptorSetLayout() {
    LOG_ATTEMPT_CAT("ComputeDS", "{}>>> OCEAN_TEAL COMPUTE DESCRIPTOR WAVES CRASHING{}", OCEAN_TEAL, RESET);
    ...
    LOG_SUCCESS_CAT("ComputeDS", "{}OCEAN_TEAL COMPUTE LAYOUT @ {:p} — WAVES READY TO CRASH{}", OCEAN_TEAL, static_cast<void*>(computeDescriptorSetLayout_), RESET);
}

// createTransientCommandPool() → TITANIUM_WHITE
void VulkanPipelineManager::createTransientCommandPool() {
    LOG_ATTEMPT_CAT("TransientPool", "{}>>> TITANIUM_WHITE TRANSIENT FLEET LAUNCHING{}", TITANIUM_WHITE, RESET);
    ...
    LOG_SUCCESS_CAT("TransientPool", "{}TITANIUM_WHITE FLEET @ {:p} — READY FOR TRANSIENT STRIKES{}", TITANIUM_WHITE, static_cast<void*>(transientPool_), RESET);
}
...
// updateRayTracingDescriptorSet() → PLASMA_FUCHSIA
void VulkanPipelineManager::updateRayTracingDescriptorSet(VkDescriptorSet ds, VkAccelerationStructureKHR tlas) {
    LOG_ATTEMPT_CAT("RTDS", "{}>>> PLASMA_FUCHSIA BINDING FUSION — TLAS TO DESCRIPTOR MATRIX{}", PLASMA_FUCHSIA, RESET);
...
// createNexusPipeline() → RASPBERRY_PINK + THERMO_PINK (ALREADY PROVIDED — KEEP)
void VulkanPipelineManager::createNexusPipeline() {
    LOG_ATTEMPT_CAT("Nexus", "{}>>> AWAKENING NEXUS GPU CONSCIOUSNESS — 1x1 DECISION MATRIX{}", RASPBERRY_PINK, RESET);
    createNexusDescriptorSetLayout();
...
// createStatsPipeline() → SPEARMINT_MINT + TURQUOISE_BLUE
void VulkanPipelineManager::createStatsPipeline() {
    LOG_ATTEMPT_CAT("Stats", "{}>>> SPEARMINT_MINT + TURQUOISE_BLUE ENTROPY CALCULUS — STATS ANALYZER FORGING{}", SPEARMINT_MINT, RESET);
...
// dispatchStats() → FUCHSIA_MAGENTA + LIME_YELLOW
void VulkanPipelineManager::dispatchStats(VkCommandBuffer cmd, VkDescriptorSet statsSet) {
    if (statsPipeline_ == VK_NULL_HANDLE) [[unlikely]] {
        LOG_WARN_CAT("Stats", "{}FUCHSIA_MAGENTA DISPATCH SKIPPED — ANALYZER NOT FORGED{}", FUCHSIA_MAGENTA, RESET);
        return;
