...
VulkanRenderer::VulkanRenderer(int width, int height, void* window, const std::vector<std::string>& instanceExtensions)
    ...
    if (frames_.size() != MAX_FRAMES_IN_FLIGHT) {
        LOG_ERROR("Failed to resize frames_ to MAX_FRAMES_IN_FLIGHT ({}), current size: {}", MAX_FRAMES_IN_FLIGHT, frames_.size());
        throw std::runtime_error("Failed to initialize frames_ vector");
    }
    LOG_DEBUG("Initialized frames_ vector with size: {}", frames_.size());
    ...
    LOG_DEBUG("Initialized Vulkan instance, surface, physical device, and device");
    ...
    if (res != VK_SUCCESS) {
        LOG_ERROR("Failed to create command pool: VkResult={}", static_cast<int>(res));
    ...
    LOG_DEBUG("Swapchain initialized: extent={}x{}, imageCount={}, viewCount={}",
    ...
    if (context_.swapchainImageViews.empty()) {
        LOG_ERROR("Swapchain image views are empty after initialization");
    ...
    if (!rtPipeline_ || !rtPipelineLayout_ || !context_.rayTracingPipeline) {
        LOG_ERROR("Failed to initialize ray-tracing pipeline: pipeline={:p}, layout={:p}, context.pipeline={:p}",
    ...
    if (context_.uniformBuffers.size() != MAX_FRAMES_IN_FLIGHT) {
        LOG_ERROR("Uniform buffer count mismatch: expected {}, got {}", MAX_FRAMES_IN_FLIGHT, context_.uniformBuffers.size());
        throw std::runtime_error("Uniform buffer count mismatch");
    }
    LOG_DEBUG("Created {} uniform buffers", context_.uniformBuffers.size());

    createFramebuffers();
    createCommandBuffers();
    createSyncObjects();
    LOG_DEBUG("Created framebuffers, command buffers, and sync objects");
    ...
    // Initial transition for storageImage from UNDEFINED to GENERAL
    {
        ...
        initBarrier.image = context_.storageImage;
        ...
        LOG_DEBUG("Initial transition: storageImage UNDEFINED -> GENERAL");
    }
    ...
    // Initial transition for denoiseImage from UNDEFINED to GENERAL
    {
        ...
        initBarrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        initBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
        initBarrier.image = denoiseImage_;
        initBarrier.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1};
        vkCmdPipelineBarrier(initCmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                             0, 0, nullptr, 0, nullptr, 1, &initBarrier);
        VulkanInitializer::endSingleTimeCommands(context_, initCmd);
        LOG_DEBUG("Initial transition: denoiseImage UNDEFINED -> GENERAL");
    }

    VkSamplerCreateInfo denoiseSamplerInfo = {};
    denoiseSamplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
    denoiseSamplerInfo.pNext = nullptr;
    denoiseSamplerInfo.magFilter = VK_FILTER_LINEAR;
    denoiseSamplerInfo.minFilter = VK_FILTER_LINEAR;
    denoiseSamplerInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_NEAREST;
    denoiseSamplerInfo.addressModeU = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    denoiseSamplerInfo.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    denoiseSamplerInfo.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    denoiseSamplerInfo.anisotropyEnable = VK_FALSE;
    denoiseSamplerInfo.minLod = 0.0f;
    denoiseSamplerInfo.maxLod = 0.0f;
    if (vkCreateSampler(context_.device, &denoiseSamplerInfo, nullptr, &denoiseSampler_) != VK_SUCCESS) {
        LOG_ERROR("Failed to create denoise sampler");
        throw std::runtime_error("Failed to create denoise sampler");
    }
    LOG_DEBUG("Created denoise sampler: {:p}", static_cast<void*>(denoiseSampler_));

    createEnvironmentMap();
    LOG_DEBUG("Created environment map");
    ...
    LOG_DEBUG("Created TLAS: {:p}", static_cast<void*>(context_.topLevelAS));
    ...
    poolInfo_desc.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo_desc.pNext = nullptr;
    poolInfo_desc.flags = 0;
    poolInfo_desc.maxSets = MAX_FRAMES_IN_FLIGHT * 3;
    poolInfo_desc.poolSizeCount = static_cast<uint32_t>(std::size(poolSizes));
    poolInfo_desc.pPoolSizes = poolSizes;
    ...
    LOG_DEBUG("Created descriptor pool: {:p}", static_cast<void*>(context_.descriptorPool));
    ...
    rayTracingAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    rayTracingAllocInfo.pNext = nullptr;
    rayTracingAllocInfo.descriptorPool = context_.descriptorPool;
    rayTracingAllocInfo.descriptorSetCount = MAX_FRAMES_IN_FLIGHT;
    rayTracingAllocInfo.pSetLayouts = rayTracingLayouts.data();
    VkDescriptorSetAllocateInfo graphicsAllocInfo = {};
    graphicsAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    graphicsAllocInfo.pNext = nullptr;
    graphicsAllocInfo.descriptorPool = context_.descriptorPool;
    graphicsAllocInfo.descriptorSetCount = MAX_FRAMES_IN_FLIGHT;
    ...
    computeAllocInfo.pNext = nullptr;
    computeAllocInfo.descriptorPool = context_.descriptorPool;
    ...
    if (vkAllocateDescriptorSets(context_.device, &rayTracingAllocInfo, rayTracingSets.data()) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate ray-tracing descriptor sets");
    ...
    if (vkAllocateDescriptorSets(context_.device, &graphicsAllocInfo, graphicsSets.data()) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate graphics descriptor sets");
...
void VulkanRenderer::createSwapchain(int width, int height) {
    LOG_DEBUG("Creating swapchain with width={}, height={}", width, height);
    if (!context_.surface) {
...
void VulkanRenderer::createEnvironmentMap() {
    ...
    if (vkCreateImage(context_.device, &imageInfo, nullptr, &envMapImage_) != VK_SUCCESS) {
        LOG_ERROR("Failed to create environment map image");
        throw std::runtime_error("Failed to create environment map image");
    }
    context_.resourceManager.addImage(envMapImage_);
    LOG_DEBUG("Created environment map image: {:p}", static_cast<void*>(envMapImage_));
    ...
    allocInfo.memoryTypeIndex = VulkanInitializer::findMemoryType(context_.physicalDevice, memRequirements.memoryTypeBits,
                                                                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (vkAllocateMemory(context_.device, &allocInfo, nullptr, &envMapImageMemory_) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate environment map memory");
    ...
    if (vkBindImageMemory(context_.device, envMapImage_, envMapImageMemory_, 0) != VK_SUCCESS) {
        LOG_ERROR("Failed to bind environment map image memory");
        throw std::runtime_error("Failed to bind environment map image memory");
    }
    LOG_DEBUG("Allocated and bound environment map memory: {:p}", static_cast<void*>(envMapImageMemory_));
    ...
    if (vkCreateImageView(context_.device, &viewInfo, nullptr, &envMapImageView_) != VK_SUCCESS) {
        LOG_ERROR("Failed to create environment map image view");
    ...
    LOG_DEBUG("Created environment map image view: {:p}", static_cast<void*>(envMapImageView_));
    ...
    if (vkCreateSampler(context_.device, &samplerInfo, nullptr, &envMapSampler_) != VK_SUCCESS) {
        LOG_ERROR("Failed to create environment map sampler");
    ...
    copyRegion.imageExtent = {4096, 2048, 1};
    ...
    LOG_DEBUG("Copied environment map data, generated 12 mip levels, and transitioned layout");
...
void VulkanRenderer::initializeAllBufferData(uint32_t maxFrames, VkDeviceSize materialBufferSize, VkDeviceSize dimensionBufferSize) {
    ...
    LOG_DEBUG("Material and dimension buffers created for {} frames", maxFrames);
}

void VulkanRenderer::initializeBufferData(uint32_t frameIndex, VkDeviceSize materialSize, VkDeviceSize dimensionSize) {
    LOG_DEBUG("Initializing buffer data for frame {}", frameIndex);
    if (frames_.empty()) {
        LOG_ERROR("Frames vector is empty; cannot initialize buffer data");
        throw std::runtime_error("Frames vector is empty");
    }
    if (frameIndex >= frames_.size()) {
        LOG_ERROR("Invalid frame index: {} (max: {})", frameIndex, frames_.size() - 1);
        throw std::out_of_range("Invalid frame index");
    }
    if (!context_.device) {
        LOG_ERROR("Invalid device handle in initializeBufferData");
        throw std::runtime_error("Invalid device handle");
    }

    vkDeviceWaitIdle(context_.device);
    LOG_DEBUG("Device idle for buffer initialization");
    ...
    if (vkMapMemory(context_.device, materialStagingBufferMemory, 0, requiredMaterialSize, 0, &data) != VK_SUCCESS) {
        LOG_ERROR("Failed to map material staging buffer memory");
    ...
    LOG_DEBUG("Mapped and copied material data to staging buffer");
    ...
    LOG_DEBUG("Copied material data to buffer[{}]: {:p}", frameIndex, static_cast<void*>(materialBuffers_[frameIndex]));
    ...
    LOG_DEBUG("Cleaned up material staging buffer and memory");
    ...
    LOG_DEBUG("Mapped and copied dimension data to staging buffer");

    VulkanInitializer::copyBuffer(context_.device, context_.commandPool, context_.graphicsQueue,
                                 dimensionStagingBuffer, dimensionBuffers_[frameIndex], requiredDimensionSize);
    LOG_DEBUG("Copied dimension data to buffer[{}]: {:p}", frameIndex, static_cast<void*>(dimensionBuffers_[frameIndex]));
...
void VulkanRenderer::updateDescriptorSetForFrame(uint32_t frameIndex, VkAccelerationStructureKHR tlas) {
    LOG_DEBUG("Updating descriptor set for frame {} with TLAS: {:p}", frameIndex, static_cast<void*>(tlas));
    if (frameIndex >= frames_.size()) {
        LOG_ERROR("Invalid frame index: {} (max: {})", frameIndex, frames_.size() - 1);
        throw std::out_of_range("Invalid frame index");
    }
    if (tlas == VK_NULL_HANDLE) {
        LOG_ERROR("Invalid TLAS for frame {}: {:p}", frameIndex, static_cast<void*>(tlas));
        throw std::runtime_error("Invalid TLAS");
    }

    VkDescriptorSet descriptorSet = frames_[frameIndex].rayTracingDescriptorSet;
    if (descriptorSet == VK_NULL_HANDLE) {
        LOG_ERROR("Null descriptor set for frame {}", frameIndex);
    ...
    accelDescriptor.pNext = nullptr;
    accelDescriptor.accelerationStructureCount = 1;
    accelDescriptor.pAccelerationStructures = &tlas;

    VkDescriptorImageInfo storageImageInfo = {};
    storageImageInfo.imageView = context_.storageImageView;
    storageImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo envMapInfo = {};
    envMapInfo.imageView = envMapImageView_;
    envMapInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    envMapInfo.sampler = envMapSampler_;

    VkDescriptorBufferInfo uniformBufferInfo = {};
    uniformBufferInfo.buffer = context_.uniformBuffers[frameIndex];
    uniformBufferInfo.offset = 0;
    uniformBufferInfo.range = sizeof(UE::UniformBufferObject);

    VkDescriptorBufferInfo materialBufferInfo = {};
    materialBufferInfo.buffer = materialBuffers_[frameIndex];
    materialBufferInfo.offset = 0;
    materialBufferInfo.range = sizeof(MaterialData) * 128;

    VkDescriptorBufferInfo dimensionBufferInfo = {};
    dimensionBufferInfo.buffer = dimensionBuffers_[frameIndex];
    dimensionBufferInfo.offset = 0;
    dimensionBufferInfo.range = sizeof(UE::DimensionData) * 1;
    ...
    LOG_DEBUG("Updated descriptor set {:p} for frame {}", static_cast<void*>(descriptorSet), frameIndex);
}

void VulkanRenderer::updateGraphicsDescriptorSet(uint32_t frameIndex) {
    LOG_DEBUG("Updating graphics descriptor set for frame {}", frameIndex);
    if (frameIndex >= frames_.size()) {
        LOG_ERROR("Invalid frame index for graphics update: {}", frameIndex);
        return;
    }
    VkDescriptorSet descSet = frames_[frameIndex].graphicsDescriptorSet;
    if (descSet == VK_NULL_HANDLE) {
        LOG_ERROR("Null graphics descriptor set for frame {}", frameIndex);
        return;
    }
    ...
    imageInfo.imageView = denoiseImageView_;
    imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    imageInfo.sampler = denoiseSampler_;
    ...
    LOG_DEBUG("Updated graphics descriptor set {:p} for frame {} with denoise image", static_cast<void*>(descSet), frameIndex);
}

void VulkanRenderer::updateComputeDescriptorSet(uint32_t frameIndex) {
    LOG_DEBUG("Updating compute descriptor set for frame {}", frameIndex);
    if (frameIndex >= frames_.size()) {
    ...
    inputInfo.imageView = context_.storageImageView;
    inputInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo outputInfo = {};
    outputInfo.imageView = denoiseImageView_;
    ...
    LOG_DEBUG("Updated compute descriptor set {:p} for frame {} with input/output images", static_cast<void*>(descSet), frameIndex);
}

void VulkanRenderer::updateDescriptorSetForTLAS(VkAccelerationStructureKHR tlas) {
    LOG_DEBUG("Updating TLAS descriptor for all frames");
    if (!tlas) {
        LOG_ERROR("Invalid TLAS handle: {:p}", static_cast<void*>(tlas));
        throw std::runtime_error("Invalid TLAS handle");
    }
    for (uint32_t i = 0; i < frames_.size(); ++i) {
        if (!frames_[i].rayTracingDescriptorSet) {
            LOG_ERROR("Invalid ray tracing descriptor set for frame {}", i);
        ...
        tlasWrite.dstSet = frames_[i].rayTracingDescriptorSet;
        tlasWrite.dstBinding = static_cast<uint32_t>(DescriptorBindings::TLAS);
        ...
        LOG_DEBUG("Updated TLAS descriptor for frame {}: tlas={:p}", i, static_cast<void*>(tlas));
    }
}
...
void VulkanRenderer::renderFrame(const Camera& camera) {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        LOG_DEBUG("Swapchain out of date, recreating");
        handleResize(width_, height_);
        return;
    } else if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
        LOG_ERROR("Failed to acquire swapchain image: result={}", static_cast<int>(result));
        throw std::runtime_error("Failed to acquire swapchain image");
    }
    if (imageIndex >= context_.framebuffers.size()) {
        LOG_ERROR("Invalid framebuffer index: {} (size={})", imageIndex, context_.framebuffers.size());
        throw std::runtime_error("Invalid framebuffer index");
    }
    LOG_DEBUG("Acquired swapchain image index: {}", imageIndex);

    if (vkResetCommandBuffer(frames_[currentFrame_].commandBuffer, 0) != VK_SUCCESS) {
        LOG_ERROR("Failed to reset command buffer for frame {}", currentFrame_);
    ...
    if (!uniformBufferMemory) {
        LOG_ERROR("Invalid uniform buffer memory for frame {}", currentFrame_);
    ...
    LOG_DEBUG("Updated uniform buffer for frame {}", currentFrame_);
    ...
    graphicsBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    graphicsBarrier.pNext = nullptr;
    graphicsBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
    graphicsBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
    graphicsBarrier.oldLayout = VK_IMAGE_LAYOUT_GENERAL;
    graphicsBarrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    graphicsBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    graphicsBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    ...
    renderPassInfo.renderPass = pipelineManager_->getRenderPass();
    ...
    postGraphicsBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    ...
    VkSemaphore waitSemaphores[] = {frames_[currentFrame_].imageAvailableSemaphore};
    ...
    VkSemaphore signalSemaphores[] = {frames_[currentFrame_].renderFinishedSemaphore};
    ...
    if (presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR) {
        LOG_DEBUG("Swapchain out of date or suboptimal, recreating");
        handleResize(width_, height_);
    } else if (presentResult != VK_SUCCESS) {
        LOG_ERROR("Failed to present queue: VkResult={}", static_cast<int>(presentResult));
    ...
    LOG_INFO("Frame {} rendered successfully", frameCount_);
}
...
void VulkanRenderer::recordRayTracingCommands(VkCommandBuffer commandBuffer, VkExtent2D extent, VkImage outputImage,
    ...
    LOG_INFO("Dispatched ray tracing: extent={}x{}", extent.width, extent.height);
}
...
void VulkanRenderer::denoiseImage(VkCommandBuffer commandBuffer, VkImage inputImage, VkImageView inputImageView,
    ...
    LOG_DEBUG("Output barrier: denoiseImage (fence-synced) -> Compute write");
...
void VulkanRenderer::createFramebuffers() {
    LOG_DEBUG("Creating {} framebuffers", swapchainManager_->getSwapchainImageViews().size());
    context_.framebuffers.resize(swapchainManager_->getSwapchainImageViews().size());
    if (context_.framebuffers.empty()) {
        LOG_WARNING("No framebuffers created due to empty swapchain image views");
        return;
    }
    for (size_t i = 0; i < context_.framebuffers.size(); ++i) {
        ...
        framebufferInfo.renderPass = pipelineManager_->getRenderPass();
        framebufferInfo.attachmentCount = 1;
        framebufferInfo.pAttachments = attachments;
        framebufferInfo.width = swapchainManager_->getSwapchainExtent().width;
        framebufferInfo.height = swapchainManager_->getSwapchainExtent().height;
        ...
        LOG_DEBUG("Created framebuffer[{}]: {:p}", i, static_cast<void*>(context_.framebuffers[i]));
    }
}
...
void VulkanRenderer::createCommandBuffers() {
    ...
    if (!swapchainManager_) {
        LOG_ERROR("Swapchain manager is null");
    ...
    LOG_DEBUG("swapchainManager_ = {:p}", static_cast<void*>(swapchainManager_.get()));
    LOG_DEBUG("Creating command buffers for {} swapchain images", swapchainManager_->getSwapchainImages().size());
    auto& imageViews = swapchainManager_->getSwapchainImageViews();
    LOG_DEBUG("Using {} image views for command buffers", imageViews.size());
    size_t imageViewCount = imageViews.size();
    if (imageViewCount == 0) {
        LOG_ERROR("No swapchain image views available. Check swapchain initialization.");
        throw std::runtime_error("Empty swapchain image views");
    }
    LOG_DEBUG("Swapchain has {} image views", imageViewCount);
    ...
    if (result != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate command buffers: VkResult={}", static_cast<int>(result));
        throw std::runtime_error("Failed to allocate command buffers");
    }
    for (size_t i = 0; i < context_.commandBuffers.size(); ++i) {
        if (context_.commandBuffers[i] == VK_NULL_HANDLE) {
            LOG_ERROR("Command buffer[{}] is null after allocation", i);
            throw std::runtime_error("Null command buffer allocated");
        }
        LOG_DEBUG("Allocated command buffer[{}]: {:p}", i, static_cast<void*>(context_.commandBuffers[i]));
    }
    if (context_.commandBuffers.empty()) {
        LOG_ERROR("No command buffers allocated");
...
void VulkanRenderer::handleResize(int width, int height) {
    LOG_DEBUG("Handling resize to {}x{}", width, height);
    if (width <= 0 || height <= 0) {
        LOG_WARNING("Invalid resize dimensions: {}x{}", width, height);
        return;
    }
    std::lock_guard<std::mutex> lock(Vulkan::cleanupMutex);
    vkDeviceWaitIdle(context_.device);
    LOG_DEBUG("Device idle for resize");
    ...
    LOG_DEBUG("Destroyed existing framebuffers");

    createFramebuffers();
    LOG_DEBUG("Recreated framebuffers");

    if (!bufferManager_) {
        LOG_ERROR("Buffer manager is null during resize");
        throw std::runtime_error("Buffer manager is null");
    }
    bufferManager_->createUniformBuffers(MAX_FRAMES_IN_FLIGHT);
    LOG_DEBUG("Recreated uniform buffers for {} frames", MAX_FRAMES_IN_FLIGHT);
    ...
    if (context_.storageImage) {
        ...
        LOG_DEBUG("Destroyed old storage image");
    }
    if (context_.storageImageMemory) {
        ...
        LOG_DEBUG("Freed old storage image memory");
    }
    if (context_.storageImageView) {
        ...
        LOG_DEBUG("Destroyed old storage image view");
    }
    VulkanInitializer::createStorageImage(context_.device, context_.physicalDevice, context_.storageImage,
    ...
    if (denoiseImage_) {
        ...
        vkDestroyImage(context_.device, denoiseImage_, nullptr);
        denoiseImage_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old denoise image");
    }
    if (denoiseImageMemory_) {
        context_.resourceManager.removeMemory(denoiseImageMemory_);
        vkFreeMemory(context_.device, denoiseImageMemory_, nullptr);
        denoiseImageMemory_ = VK_NULL_HANDLE;
        LOG_DEBUG("Freed old denoise image memory");
    }
    if (denoiseImageView_) {
        context_.resourceManager.removeImageView(denoiseImageView_);
        vkDestroyImageView(context_.device, denoiseImageView_, nullptr);
        denoiseImageView_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old denoise image view");
    }
    if (denoiseSampler_) {
        vkDestroySampler(context_.device, denoiseSampler_, nullptr);
        denoiseSampler_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old denoise sampler");
    }
    VulkanInitializer::createStorageImage(context_.device, context_.physicalDevice, denoiseImage_,
    ...
    denoiseSamplerInfo.maxLod = 0.0f;
    if (vkCreateSampler(context_.device, &denoiseSamplerInfo, nullptr, &denoiseSampler_) != VK_SUCCESS) {
        LOG_ERROR("Failed to recreate denoise sampler");
        throw std::runtime_error("Failed to recreate denoise sampler");
    }
    LOG_DEBUG("Recreated denoise sampler: {:p}", static_cast<void*>(denoiseSampler_));

    if (envMapImage_) {
        context_.resourceManager.removeImage(envMapImage_);
        vkDestroyImage(context_.device, envMapImage_, nullptr);
        envMapImage_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old environment map image");
    }
    if (envMapImageMemory_) {
        context_.resourceManager.removeMemory(envMapImageMemory_);
        vkFreeMemory(context_.device, envMapImageMemory_, nullptr);
        envMapImageMemory_ = VK_NULL_HANDLE;
        LOG_DEBUG("Freed old environment map image memory");
    }
    if (envMapImageView_) {
        context_.resourceManager.removeImageView(envMapImageView_);
        vkDestroyImageView(context_.device, envMapImageView_, nullptr);
        envMapImageView_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old environment map image view");
    }
    if (envMapSampler_) {
        vkDestroySampler(context_.device, envMapSampler_, nullptr);
        envMapSampler_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old environment map sampler");
    }
    createEnvironmentMap();
    LOG_DEBUG("Recreated environment map (high-res)");
    ...
    frames_.resize(MAX_FRAMES_IN_FLIGHT);
    LOG_DEBUG("Resized frames_ to size: {}", frames_.size());
    if (frames_.size() != MAX_FRAMES_IN_FLIGHT) {
        LOG_ERROR("Failed to resize frames_ to MAX_FRAMES_IN_FLIGHT ({}), current size: {}", MAX_FRAMES_IN_FLIGHT, frames_.size());
    ...
    if (context_.descriptorPool) {
        ...
        LOG_DEBUG("Destroyed old descriptor pool");
    }
    VkDescriptorPoolSize poolSizes[] = {
        {VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, MAX_FRAMES_IN_FLIGHT},
        {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, MAX_FRAMES_IN_FLIGHT * 3},
    ...
    poolInfo_desc.pPoolSizes = poolSizes;
    if (vkCreateDescriptorPool(context_.device, &poolInfo_desc, nullptr, &context_.descriptorPool) != VK_SUCCESS) {
        LOG_ERROR("Failed to recreate descriptor pool during resize");
        throw std::runtime_error("Failed to recreate descriptor pool");
    }
    context_.resourceManager.addDescriptorPool(context_.descriptorPool);
    LOG_DEBUG("Recreated descriptor pool: {:p}", static_cast<void*>(context_.descriptorPool));

    if (computeDescriptorSetLayout_) {
        context_.resourceManager.removeDescriptorSetLayout(computeDescriptorSetLayout_);
        vkDestroyDescriptorSetLayout(context_.device, computeDescriptorSetLayout_, nullptr);
        computeDescriptorSetLayout_ = VK_NULL_HANDLE;
        LOG_DEBUG("Destroyed old compute descriptor set layout");
    }
    VkDescriptorSetLayoutBinding computeBindings[] = {
        {0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr},
        {1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr}
    ...
    computeLayoutInfo.pBindings = computeBindings;
    if (vkCreateDescriptorSetLayout(context_.device, &computeLayoutInfo, nullptr, &computeDescriptorSetLayout_) != VK_SUCCESS) {
        LOG_ERROR("Failed to recreate compute descriptor set layout during resize");
        throw std::runtime_error("Failed to recreate compute descriptor set layout");
    }
    context_.resourceManager.addDescriptorSetLayout(computeDescriptorSetLayout_);
    LOG_DEBUG("Recreated compute descriptor set layout: {:p}", static_cast<void*>(computeDescriptorSetLayout_));
    ...
    std::vector<VkDescriptorSet> computeSets(MAX_FRAMES_IN_FLIGHT);
    if (vkAllocateDescriptorSets(context_.device, &rayTracingAllocInfo, rayTracingSets.data()) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate ray-tracing descriptor sets during resize");
        throw std::runtime_error("Failed to allocate ray-tracing descriptor sets");
    }
    if (vkAllocateDescriptorSets(context_.device, &graphicsAllocInfo, graphicsSets.data()) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate graphics descriptor sets during resize");
        throw std::runtime_error("Failed to allocate graphics descriptor sets");
    }
    if (vkAllocateDescriptorSets(context_.device, &computeAllocInfo, computeSets.data()) != VK_SUCCESS) {
        LOG_ERROR("Failed to allocate compute descriptor sets during resize");
        throw std::runtime_error("Failed to allocate compute descriptor sets");
    }
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        frames_[i].rayTracingDescriptorSet = rayTracingSets[i];
    ...
    LOG_DEBUG("Freed existing command buffers");
    for (size_t i = 0; i < context_.imageAvailableSemaphores.size(); ++i) {
        if (context_.imageAvailableSemaphores[i]) {
            ...
            LOG_DEBUG("Destroyed image available semaphore[{}]", i);
        }
        if (context_.renderFinishedSemaphores[i]) {
            ...
            LOG_DEBUG("Destroyed render finished semaphore[{}]", i);
        }
        if (context_.inFlightFences[i]) {
            ...
            LOG_DEBUG("Destroyed in-flight fence[{}]", i);
        }
    }
    createCommandBuffers();
    createSyncObjects();
    LOG_DEBUG("Recreated command buffers and sync objects");

    width_ = width;
    height_ = height;
    LOG_INFO("VulkanRenderer resized successfully to {}x{}", width, height);
}

void VulkanRenderer::createAccelerationStructures() {
    LOG_DEBUG("Creating acceleration structures");
    std::vector<glm::vec3> vertices = getVertices();
    std::vector<uint32_t> indices = getIndices();
    if (vertices.empty() || indices.empty()) {
        LOG_ERROR("Empty vertex or index data for acceleration structure creation");
