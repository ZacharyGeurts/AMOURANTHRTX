...
// FIXED: Ambiguous logAndTrackDestruction calls → non-static definition
// FIXED: All logging uses std::format — NO %s, NO printf-style issues
// FIXED: Double-free message clean and consistent
// FIXED: reinterpret_cast for opaque handles (no uintptr_t intermediate)
...
void logAndTrackDestruction(std::string_view typeName, void* ptr) {
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Destroying {} {}{}", Color::EMERALD_GREEN, typeName, ptr, Color::RESET));
}
    ...
    for (auto it = container.rbegin(); it != container.rend(); ++it) {
        ...
        if (DestroyTracker::isDestroyed(ptr)) {
            LOG_ERROR_CAT("Dispose",
                std::format("{}DOUBLE FREE DETECTED! {} {} (name: '{}') already destroyed — skipping!{}",
        ...
        try {
            ...
            *it = VK_NULL_HANDLE;
            ...
            LOG_ERROR_CAT("Dispose",
                std::format("EXCEPTION destroying {} {}: {}", typeName, ptr, e.what()));
        } catch (...) {
            LOG_ERROR_CAT("Dispose",
                std::format("UNKNOWN EXCEPTION destroying {} {}", typeName, ptr));
        }
    }
    container.clear();
}
...
void cleanupAll(Vulkan::Context& ctx) noexcept {
    ...
    LOG_INFO_CAT("Dispose",
        std::format("{}cleanupAll() — START (thread: {}){}",
                   Color::EMERALD_GREEN, threadId, Color::RESET));

    if (ctx.device == VK_NULL_HANDLE) {
        LOG_INFO_CAT("Dispose",
            std::format("{}Device handle is null — early exit{}", Color::OCEAN_TEAL, Color::RESET));
        return;
    }

    LOG_INFO_CAT("Dispose",
        std::format("{}Skipping vkDeviceWaitIdle() — device may be lost{}", Color::OCEAN_TEAL, Color::RESET));

    // Phase 1 – Shader Modules
    LOG_INFO_CAT("Dispose", std::format("{}Phase 1: Shader Modules{}", Color::ARCTIC_CYAN, Color::RESET));
    safeDestroyContainer(ctx.resourceManager.getShaderModulesMutable(),
                         vkDestroyShaderModule, "ShaderModule", ctx.device);

    // Phase 2 – Pipelines
    LOG_INFO_CAT("Dispose", std::format("{}Phase 2: Pipelines{}", Color::ARCTIC_CYAN, Color::RESET));
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 3: Descriptor Set Layouts{}", Color::ARCTIC_CYAN, Color::RESET));
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 4: Render Passes{}", Color::ARCTIC_CYAN, Color::RESET));
    safeDestroyContainer(ctx.resourceManager.getRenderPassesMutable(),
                         vkDestroyRenderPass, "RenderPass", ctx.device);

    // Phase 5 – Buffers
    LOG_INFO_CAT("Dispose", std::format("{}Phase 5: Buffers{}", Color::ARCTIC_CYAN, Color::RESET));
    safeDestroyContainer(ctx.resourceManager.getBuffersMutable(),
                         vkDestroyBuffer, "Buffer", ctx.device);

    // Phase 6 – Command Pools
    LOG_INFO_CAT("Dispose", std::format("{}Phase 6: Command Pools{}", Color::ARCTIC_CYAN, Color::RESET));
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 7: Swapchain Images{}", Color::ARCTIC_CYAN, Color::RESET));

    // Phase 8 – Swapchain
    LOG_INFO_CAT("Dispose", std::format("{}Phase 8: Swapchain{}", Color::ARCTIC_CYAN, Color::RESET));
    if (ctx.swapchain != VK_NULL_HANDLE) {
        void* ptr = handleToPtr(reinterpret_cast<uintptr_t>(ctx.swapchain));
        if (!DestroyTracker::isDestroyed(ptr)) {
            logAndTrackDestruction("Swapchain", ptr);
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 9: Surface{}", Color::ARCTIC_CYAN, Color::RESET));
    if (ctx.surface != VK_NULL_HANDLE) {
        void* ptr = handleToPtr(reinterpret_cast<uintptr_t>(ctx.surface));
        if (!DestroyTracker::isDestroyed(ptr)) {
            logAndTrackDestruction("Surface", ptr);
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 10: Device{}", Color::ARCTIC_CYAN, Color::RESET));
    if (ctx.device != VK_NULL_HANDLE) {
    ...
    LOG_INFO_CAT("Dispose", std::format("{}Phase 11: Instance{}", Color::ARCTIC_CYAN, Color::RESET));
    ...
    LOG_INFO_CAT("Dispose",
        std::format("{}cleanupAll() — COMPLETE — {} handles destroyed{}",
