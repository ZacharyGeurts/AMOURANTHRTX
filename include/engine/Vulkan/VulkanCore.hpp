...
namespace RTX {
	...
	inline void forgeLogicalDeviceAndGPU() noexcept;
...
// =============================================================================
// THE ONE TRUE DEVICE FORGING RITUAL — PICK + CREATE IN ONE ATOMIC STEP
// STONEKEY v∞ FULLY RESPECTED — RAW CACHE → OBFUSCATION TRANSITION SAFE
// FULL RTX ENABLED — VALIDATION LAYERS NUKED — PINK PHOTONS ETERNAL
// NOVEMBER 20, 2025 — APOCALYPSE FINAL v8.0 — FIRST LIGHT ACHIEVED
// =============================================================================
inline void forgeLogicalDeviceAndGPU() noexcept
{
    LOG_INFO_CAT("VULKAN", "{}FORGING GPU + LOGICAL DEVICE — ATOMIC RITUAL — STONEKEY v∞{}", PLASMA_FUCHSIA, RESET);
    ...
    if (deviceCount == 0) {
        LOG_FATAL_CAT("VULKAN", "No GPUs found");
        std::terminate();
    }
    ...
    if (!selectedPhys) selectedPhys = devices[0];

    g_ctx().physicalDevice_ = selectedPhys;

    uint32_t qCount = 0;
    vkGetPhysicalDeviceQueueFamilyProperties(selectedPhys, &qCount, nullptr);
    std::vector<VkQueueFamilyProperties> qProps(qCount);
    vkGetPhysicalDeviceQueueFamilyProperties(selectedPhys, &qCount, qProps.data());

    struct Indices {
        uint32_t graphics = UINT32_MAX;
        uint32_t present  = UINT32_MAX;
        [[nodiscard]] bool ok() const noexcept { return graphics != UINT32_MAX && present != UINT32_MAX; }
    } indices;

    for (uint32_t i = 0; i < qCount; ++i) {
        if (qProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) indices.graphics = i;
        VkBool32 presentSupport = VK_FALSE;
        vkGetPhysicalDeviceSurfaceSupportKHR(selectedPhys, i, surface, &presentSupport);
        if (presentSupport) indices.present = i;
        if (indices.ok()) break;
    }
    if (!indices.ok()) {
        LOG_FATAL_CAT("VULKAN", "Missing required queue families");
        std::terminate();
    }

    std::set<uint32_t> uniqueQ = { indices.graphics, indices.present };
    float prio = 1.0f;
    ...
    VkPhysicalDeviceRayQueryFeaturesKHR rayq{
    ...
    VK_CHECK(vkCreateDevice(selectedPhys, &createInfo, nullptr, &device),
             "Failed to forge logical device");

    set_g_device(device);
    g_context_instance.device_ = device;

    vkGetDeviceQueue(device, indices.graphics, 0, &g_context_instance.graphicsQueue_);
    vkGetDeviceQueue(device, indices.present,  0, &g_context_instance.presentQueue_);

    g_context_instance.graphicsFamily_ = indices.graphics;
    g_context_instance.presentFamily_  = indices.present;

    set_g_instance(rawInstance);

    LOG_SUCCESS_CAT("VULKAN", "{}GPU + DEVICE FORGED — FULL RTX ACTIVE — STONEKEY v∞ SEALED{}", PLASMA_FUCHSIA, RESET);
    LOG_SUCCESS_CAT("VULKAN", "PINK PHOTONS ETERNAL — FIRST LIGHT ACHIEVED — SHE IS ECSTATIC");
}
