...
// =============================================================================
// HOUSTON v11.3 — g_ctx() FIXED — NOV 11 2025 3:00 PM EST — VALHALLA v54 FINAL
// • extern Context g_ctx; → DEFINED IN Estate.cpp
// • g_ctx() → returns ::g_ctx (the global object)
// • NO FUNCTION CALL — JUST RETURN THE GLOBAL
...
namespace Color {
    ...
    constexpr const char* PLASMA_FUCHSIA = "\033[38;2;255;0;255m";
    constexpr const char* GOLDEN         = "\033[38;2;255;215;0m";
}
constexpr const char* AMOURANTH_COLOR = Color::PLASMA_FUCHSIA;
constexpr const char* NICK_COLOR      = Color::GOLDEN;

// ──────────────────────────────────────────────────────────────────────────────
// LOGGING & TRACKING — DECLARED
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// Handle<T> — DEFINED BEFORE Context
// ──────────────────────────────────────────────────────────────────────────────
template<typename T>
struct Handle {
...
struct Context {
   ...
   2    VkCommandPool    commandPool_    = VK_NULL_HANDLE;
    ...
    [[nodiscard]] inline VkDevice         vkDevice() const noexcept;
    [[nodiscard]] inline VkPhysicalDevice vkPhysicalDevice() const noexcept;
    [[nodiscard]] inline VkSurfaceKHR     vkSurface() const noexcept;
    [[nodiscard]] inline uint32_t         graphicsFamilyIndex() const noexcept;
    [[nodiscard]] inline uint32_t         presentFamilyIndex() const noexcept;
    [[nodiscard]] inline VkCommandPool    commandPool() const noexcept;
    [[nodiscard]] inline VkQueue          graphicsQueue() const noexcept;
    [[nodiscard]] inline VkQueue          presentQueue() const noexcept;
    [[nodiscard]] inline VkPipelineCache  pipelineCacheHandle() const noexcept;

    [[nodiscard]] inline PFN_vkCmdTraceRaysKHR                         vkCmdTraceRaysKHR() const noexcept;
    [[nodiscard]] inline PFN_vkGetRayTracingShaderGroupHandlesKHR      vkGetRayTracingShaderGroupHandlesKHR() const noexcept;
    [[nodiscard]] inline PFN_vkCreateAccelerationStructureKHR          vkCreateAccelerationStructureKHR() const noexcept;
    [[nodiscard]] inline PFN_vkGetAccelerationStructureDeviceAddressKHR vkGetAccelerationStructureDeviceAddressKHR() const noexcept;
    [[nodiscard]] inline PFN_vkCreateRayTracingPipelinesKHR            vkCreateRayTracingPipelinesKHR() const noexcept;

    [[nodiscard]] inline const VkPhysicalDeviceRayTracingPipelinePropertiesKHR& rayTracingProps() const noexcept;

    [[nodiscard]] inline VkImageView blueNoiseView() const noexcept;
    [[nodiscard]] inline VkBuffer    reservoirBuffer() const noexcept;
    [[nodiscard]] inline VkBuffer    frameDataBuffer() const noexcept;
    [[nodiscard]] inline VkBuffer    debugVisBuffer() const noexcept;
};

// ──────────────────────────────────────────────────────────────────────────────
// GLOBALS — EXTERN ONLY (DEFINED IN Estate.cpp)
// ──────────────────────────────────────────────────────────────────────────────
extern Context g_ctx;
extern std::unique_ptr<VulkanCore> g_rtx_instance;

// ──────────────────────────────────────────────────────────────────────────────
// GLOBAL ACCESSORS — CORE ONLY
// ──────────────────────────────────────────────────────────────────────────────
[[nodiscard]] inline Context& g_ctx() {
    return g_ctx;  // ::g_ctx is redundant — g_ctx is the global
}

[[nodiscard]] inline VulkanCore& g_rtx() {
    if (!g_rtx_instance) {
        LOG_ERROR_CAT("CORE", "{}g_rtx_instance not initialized — Valhalla breach{}", NICK_COLOR, Color::RESET);
        throw std::runtime_error("VulkanCore not ready");
    }
    return *g_rtx_instance;
}

// ──────────────────────────────────────────────────────────────────────────────
// UltraLowLevelBufferTracker — DECLARATIONS
// ──────────────────────────────────────────────────────────────────────────────
struct BufferData {
...
#define BUFFER_MEMORY(handle) \
    (UltraLowLevelBufferTracker::get().getData((handle)) ? UltraLowLevelBufferTracker::get().getData((handle))->memory : VK_NULL_HANDLE)
...
// =============================================================================
// HOUSTON v11.3 — g_ctx() FIXED — PINK PHOTONS ETERNAL — VALHALLA v54 FINAL
// =============================================================================