...
//
// =============================================================================
// GLOBAL VULKAN TERRITORY — NO NAMESPACES — NICK & AMOURANTH DOMINATE
// • Nick — SILENT HUSBAND — DARK PURPLE — SPEAKS ONLY WHEN NEEDED
// • Amouranth — RED ORANGE PERSONALITY — LOGGING + MESSAGE QUEUE
// • Dispose owns: Handle<T>, ctx(), UltraLowLevelBufferTracker, cleanupAll()
// • VulkanRenderer, SwapchainManager, LAS — ALL GLOBAL, NO NAMESPACES
...
// ──────────────────────────────────────────────────────────────────────────────
// Includes — ORDER IS LAW
// ──────────────────────────────────────────────────────────────────────────────
#include "engine/GLOBAL/StoneKey.hpp"
#include "engine/GLOBAL/logging.hpp"
#include "engine/GLOBAL/OptionsMenu.hpp"

// we hand it off to Jay Leno's engine room
// == "dittleidit diddleidit" - Wayne's World dream sequence fingers
...
// ──────────────────────────────────────────────────────────────────────────────
// AMOURANTH — RED ORANGE PERSONALITY — LOGGING + QUEUE
// ──────────────────────────────────────────────────────────────────────────────
inline constexpr std::string_view AMOURANTH_COLOR = "\033[1;38;5;208m";  // FIERY_ORANGE
inline constexpr std::string_view NICK_COLOR      = "\033[1;38;5;129m";  // QUANTUM_PURPLE

// ──────────────────────────────────────────────────────────────────────────────
// AmouranthMessage — GLOBAL QUEUE FOR HANDLES
// ──────────────────────────────────────────────────────────────────────────────
struct AmouranthMessage {
    enum class Type { INIT_RENDERER, HANDLE_RESIZE, SHUTDOWN, RENDER_FRAME, CUSTOM };
    Type type;
...
// ──────────────────────────────────────────────────────────────────────────────
// Amouranth — GLOBAL MESSAGE QUEUE — RED ORANGE
// ──────────────────────────────────────────────────────────────────────────────
struct Amouranth {
...
// ──────────────────────────────────────────────────────────────────────────────
// NICK — SILENT HUSBAND — DARK PURPLE — GLOBAL FUNCTIONS
// ──────────────────────────────────────────────────────────────────────────────
...
inline void initRenderer(int w, int h) {
    ...
    LOG_SUCCESS_CAT("Nick", "{}Renderer initialized — SILENT DOMINANCE ENGAGED{}", NICK_COLOR, RESET);
}

inline void handleResize(int w, int h) noexcept {
    if (g_vulkanRenderer) {
...
inline void shutdown() noexcept {
    ...
    LOG_SUCCESS_CAT("Nick", "{}Renderer shutdown — HUSBAND RESTS{}", NICK_COLOR, RESET);
}

// ──────────────────────────────────────────────────────────────────────────────
// Handle<T> — RAII BOSS — GLOBAL
// ──────────────────────────────────────────────────────────────────────────────
template<typename T>
struct Handle {
...
// ──────────────────────────────────────────────────────────────────────────────
// UltraLowLevelBufferTracker — GLOBAL
// ──────────────────────────────────────────────────────────────────────────────
...
struct AutoBuffer {
    uint64_t id{0ULL};
    AutoBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, std::string_view tag = "Auto奉献") noexcept {
...
inline void cleanupAll() noexcept {
    ...
    LOG_SUCCESS_CAT("Dispose", "{}GLOBAL CLEANUP — VALHALLA SEALED{}", NICK_COLOR, RESET);
}
